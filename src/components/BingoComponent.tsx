        ...
        <View style={style.row}>
            {
                items.map((value,index) => (
                    <TextInput style={style.box} key={index} value={value} onChangeText={text => updateFn(index, text)} />
                ))
            }
        </View>
...
const BingoComponent: React.FC<BingoProps> = ({width, height}: BingoProps) => {
    const [values, _setValues] = useState<string[]>(Array(Number(width) *  Number(height)).fill(''))
    ...
    return (
        <div style={styles.container}>
            {Array(height).fill(null).map((value, yPosition) => {
